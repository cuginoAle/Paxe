...
const { newLine, sameLine } = require('./logger')
...
function spinner () {
  ...
  return {
    start: ({ theme, label = '', iconBefore = false, trackTime = false }, processIndex) => {
      x = 0
      ...
      let msg = iconBefore ? `${frames[x]}${spinnerLabel}` : `${spinnerLabel}${frames[x]}`
      if (trackTime) {
        let now = Date.now()
        newLine(`${clockFrames[t++]}${parseInt((now - startTime) / 1000)} ${msg} `, processIndex)
        t = t % clockFrames.length
      } else {
        newLine(`${msg} `, processIndex)
      }
      ...
      intervalHnd = setInterval(function () {
        ...
        if (trackTime) {
          let now = Date.now()
          sameLine(`${clockFrames[t++]}${parseInt((now - startTime) / 1000)} ${msg} `, processIndex)
          t = t % clockFrames.length
        } else {
          sameLine(`${msg} `, processIndex)
        }
        x = x % frames.length
      }, interval)
    },
    ...
    success: (msg = 'done', icon = chalk.green('✔'), processIndex) => {
      clearInterval(intervalHnd)
      sameLine(`${icon} ${msg}`, processIndex)
    },
    fail: (msg = 'error', processIndex) => {
      clearInterval(intervalHnd)
      sameLine(`${chalk.red('✖')} ${msg}`, processIndex)
    },
    stop: (msg, processIndex) => {
      clearInterval(intervalHnd)

      if (msg) {
        newLine(`${msg}`, processIndex)
      } else {
        sameLine(``, processIndex)
      }
    },
    log: (msg, processIndex) => {
      newLine(`${msg}`, processIndex)
    }
  }
}
